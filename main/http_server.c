...
static const char upload_html[] =
    "<!DOCTYPE html><html><body><h2>Upload PNG</h2>"
    "<input id=\"file\" type=\"file\" accept=\".png\"><button "
    "onclick=\"u()\">Upload</button>"
...
static esp_err_t upload_post_handler(httpd_req_t *req) {
  ...
  int len = snprintf(filepath, sizeof(filepath), "%s/upload/%s", MOUNT_POINT,
                     clean_name);
  ...
  while (remaining > 0) {
    ssize_t received = httpd_req_recv(
        req, buf, remaining > sizeof(buf) ? sizeof(buf) : remaining);
    if (received < 0) {
